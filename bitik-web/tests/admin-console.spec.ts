...
test("admin dashboard and logs render for admin role", async ({ page }) => {
  ...
  await page.route(routeApiMatch("/api/v1/admin/dashboard/overview"), async (route) => {
    await mockJson(route, envelope({ users_total: 10, sellers_total: 2 }))
  })
  await page.route(routeApiMatch("/api/v1/admin/logs/audit"), async (route) => {
...
test("admin wave approval actions call backend", async ({ page }) => {
  ...
  await page.route(routeApiMatch("/api/v1/admin/payments/pay-1/wave/reject"), async (route) => {
...
test("admin CMS and RBAC pages render with mocked data", async ({ page }) => {
  await stubAdminSession(page, ["admin"])
  await page.route(routeApiMatch("/api/v1/admin/cms/pages"), async (route) => {
    await mockJson(route, envelope({ items: [{ id: "page-1", slug: "faq" }] }))
  })
  await page.route(routeApiMatch("/api/v1/admin/rbac/roles"), async (route) => {
...
test("admin categories and brands CRUD actions call real endpoints", async ({ page }) => {
  await stubAdminSession(page, ["admin"])
  await page.route(routeApiMatch("/api/v1/admin/categories"), async (route) => {
  ...
  await page.route(routeApiMatch("/api/v1/admin/categories/reorder"), async (route) => {
    await mockJson(route, envelope({ updated: 1 }))
  })
  await page.route(routeApiMatch("/api/v1/admin/categories/"), async (route) => {
    await mockJson(route, envelope({ updated: true }))
  })
  await page.route(routeApiMatch("/api/v1/admin/brands"), async (route) => {
    if (route.request().method() === "GET") {
...
test("admin products page uses admin product source and detail endpoint", async ({ page }) => {
  await stubAdminSession(page, ["admin"])
  await page.route(routeApiMatch("/api/v1/admin/products"), async (route) => {
    await mockJson(route, envelope({ items: [{ id: "prod-1", name: "Shoe" }] }))
  })
  await page.route(routeApiMatch("/api/v1/admin/products/prod-1"), async (route) => {
    await mockJson(route, envelope({ id: "prod-1", name: "Shoe", slug: "shoe" }))
  })
  await page.route(routeApiMatch("/api/v1/admin/products/prod-1/moderation"), async (route) => {
...
test("admin shipments and webhooks pages support detail/tracking/reprocess", async ({ page }) => {
  await stubAdminSession(page, ["admin"])
  await page.route(routeApiMatch("/api/v1/admin/shipping/shipments"), async (route) => {
    await mockJson(route, envelope({ items: [{ id: "ship-1", status: "pending" }] }))
  })
  await page.route(routeApiMatch("/api/v1/admin/shipping/shipments/ship-1"), async (route) => {
    await mockJson(route, envelope({ id: "ship-1", status: "pending" }))
  })
  await page.route(routeApiMatch("/api/v1/admin/shipping/shipments/ship-1/tracking"), async (route) => {
    await mockJson(route, envelope({ items: [{ id: "evt-1", status: "in_transit" }] }))
  })
  await page.route(routeApiMatch("/api/v1/admin/shipping/shipments/ship-1/status"), async (route) => {
    await mockJson(route, envelope({ id: "ship-1", status: "in_transit" }))
  })
  await page.route(routeApiMatch("/api/v1/admin/payments/webhooks"), async (route) => {
    await mockJson(route, envelope({ items: [{ id: "wh-1", event_id: "e1", provider: "wave_manual", processed: false }] }))
  })
  await page.route(routeApiMatch("/api/v1/admin/payments/webhooks/wh-1"), async (route) => {
    await mockJson(route, envelope({ id: "wh-1", event_id: "e1", provider: "wave_manual", processed: false }))
  })
  await page.route(routeApiMatch("/api/v1/admin/payments/webhooks/wh-1/reprocess"), async (route) => {
