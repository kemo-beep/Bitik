...
test("buyer chat center send and delete flows render", async ({ page }) => {
  ...
  await page.route(routeApiMatch("/api/v1/chat/conversations"), async (route) => {
  ...
  await page.route(routeApiMatch("/api/v1/chat/conversations/conv-1/messages"), async (route) => {
    if (route.request().method() === "POST") {
  ...
  await page.route(routeApiMatch("/api/v1/chat/conversations/conv-1/messages/msg-1"), async (route) => {
    await route.fulfill({ status: 204 })
  })
  await page.route(routeApiMatch("/api/v1/chat/conversations/conv-1/read"), async (route) => {
    await route.fulfill({ status: 204 })
  })
  await page.route(routeApiMatch("/api/v1/chat/conversations/conv-1"), async (route) => {
    await route.fulfill({ status: 204 })
  })
  await page.route(routeApiMatch("/api/v1/media/upload/presigned-url"), async (route) => {
  ...
  await page.route(routeApiMatch("/api/v1/media/upload/presigned-complete"), async (route) => {
...
test("buyer notifications center actions call real endpoints", async ({ page }) => {
  await stubBuyer(page)
  await page.route(routeApiMatch("/api/v1/buyer/notifications/unread-count"), async (route) => {
    await mockJson(route, envelope({ unread: 2 }))
  })
  await page.route(routeApiMatch("/api/v1/buyer/notifications/read-all"), async (route) => {
    await route.fulfill({ status: 204 })
  })
  await page.route(routeApiMatch("/api/v1/buyer/notifications/n-1/read"), async (route) => {
    await mockJson(route, envelope({ id: "n-1", read_at: new Date().toISOString() }))
  })
  await page.route(routeApiMatch("/api/v1/buyer/notifications/n-1"), async (route) => {
    await route.fulfill({ status: 204 })
  })
  await page.route(routeApiMatch("/api/v1/buyer/notifications"), async (route) => {
